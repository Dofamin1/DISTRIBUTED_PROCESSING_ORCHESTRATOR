...
class RemoteDockerDbCluster {
    constructor(imageResolver, configDir, options) {
        ...
        this.docker.command = (node, command) => {
            return dockerCLI.Docker.prototype.command.call(
                this.docker,
                [this._getHostPrefix(node.host, node.port).concat(" ", command)]
            );
        };
    ...
    run() {
        ...
        return Promise.all(
            this.nodes.map(node => {
                ...
                return this.docker.command(node, `pull ${this.imageResolver.name}`)
    ...
    _doRun = (filename, node) => this.docker.command(node, `run -p ${port}:${port} -p ${port + 10000}:${port + 10000}`
    ...
    _getHostPrefix(host, port) {
        return `-H=${host}:${port}`;
    }
}

module.exports = RemoteDockerDbCluster;